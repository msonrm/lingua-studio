...
### 実験: LinguaScript に曖昧さ解消の実用性はあるか → 示せなかった

計画 [docs/EXPERIMENT-PLAN.md](./docs/EXPERIMENT-PLAN.md) / 結果 [docs/EXPERIMENT-RESULT.md](./docs/EXPERIMENT-RESULT.md)

判定基準を実行前に固定し、**否定的な結論が出るように**組んだ。

| Phase | やったこと | 結果 |
|---|---|---|
| 0 | AST を機械的に列挙し、同じ英語になる相異なる AST を集める | 3,700 AST → 衝突1,000組。**曖昧さは2種類だけ** |
| 1 | 英語だけ与えて「解釈は何通りか」と直接聞く | **18/18 検出** |
| 2-F | 同じ文を翻訳タスクとして渡す（曖昧さに触れない） | **3/18 しか指摘しない** |
| 2-F-instructed | 「曖昧なら指摘して」の一行を足す | **14/18、主要な種類は 6/6** |

**結論**: 一行の指示が上限に達しているので、LinguaScript を与えても超えられない。
事前登録した「F が B と同等 → 不要」に該当する。

D（言い換え）・E（汎用 JSON）・B（英語+LinguaScript）は実施していない。
決定的な条件が否定的に出たあとで他を探すのは、計画で避けると決めた
「肯定を探しにいく」振る舞いそのものであるため。

#### 副産物

- **英語の構造的曖昧さは PP の係り先とモダリティ作用域の2種類のみ**（ツールの表現範囲内）。
  等位接続・否定の作用域・限定詞・時制相は英語が区別している
- **AI は能力があっても発動しない**。検出 100% に対し自発 17%、PP の係り先では 0/6。
  実用的な教訓は記法ではなく**プロンプト**にある
- **日本語のほうがはるかに曖昧**。同じ 3,700 AST から英語 2,620 通り / 日本語 1,455 通り
- Phase 0 の衝突一覧は LLM を使わず作れる。**曖昧さの「可視化」**は教材の素材になりうる

#### 実験中に見つけた問題

- 縮約が偽の区別を作っていた。`I can't eat` と `I can not eat` を別文字列として
  数えると英語の曖昧さを過小評価する（正規化して測り直した）
- 採点を正規表現で行うと、応答が触れているのが**別の曖昧さ**（能力/許可、否定の焦点）
  でも「指摘した」と数えてしまう。11/18 → 実際は 3/18 だった。LLM 判定に切り替えた
- 日本語レンダラーが限定詞 `no` を訳さず「noりんご」と英語のまま出す（既知の TODO）

### 前置詞動詞を1語として扱う

`I live a park.` の調査から、英語が項に前置詞を付けないのが分かっていた。
直すにあたり「動詞+前置詞」を2種類に分けた。
...
判定は「前置詞を差し替えて別の意味の正しい文になるか」で付く。
`It belongs to me.` の to は他にならないが、`put it on / in / under the table`
は全部成り立って意味が違う。
...
```lisp
go(agent:'I, goal:noun(head:'school))    ; "I go to school." — to は出ない
```

`fixed` の前置詞は選択の余地がなく情報量がゼロなので、`eats` の `-s` を
AST に出さないのと同じ理由で出さない。ブロックのラベルも `belong to` と
前置詞込みで見せる（前置詞とセットで覚えるのが正しい教え方のため）。

`choice` は差し替えで命題が変わるので AST に出す必要がある。まだ前置詞句を
受けるスロットが無いので、**欠けを `___` で示す**にとどめた。

```
I live ___ a park.          ← in / at / on を選ぶ必要がある
I put an apple ___ a park.  ← on / in / under で意味が変わる
```

限定詞欠損と同じ扱い。黙って `in` を補うとブロックに無いものが出力に現れる。

#### 項の追加

前置詞動詞に項が無く `I go.` しか作れなかったので、任意の項を足した。

| 動詞 | 追加した項 | 例 |
|---|---|---|
| go / come | goal | I go to school. |
| arrive | goal | I arrive at a park. |
| wait | theme | I wait for you. |
| belong | recipient | It belongs to me. |

任意（`required: false`）なので既存の文には影響しない。

#### 層の整理

前置詞は `languages/en/verbPrepositions.ts` に置いた。「どんな項を取るか」は
言語非依存だが、**それをどう標示するか**は言語ごとに違う（英語は前置詞、
日本語は格助詞）。`belong to` の to に対応するものは日本語には無い。

以前は `ArgumentSlot.preposition` として言語非依存の `concepts/` 側にあったが、
一度も設定されない死んだフィールドだったので削除した。

対になる日本語側も直した。場所の「で」は**動作が行われる場所**を指すので、
住む・居住する・滞在するは「に」が正しい（「公園で住む」→「公園に住む」）。

### ゴールデンテストの網羅性を機械的に確認する

直前の2件（VP 等位接続の項欠落・限定詞なしの可算名詞）は、どちらも
**スナップショットが1件も変わらないまま**修正が入った。テストが無いのではなく
**痩せた形でしかテストされていなかった**ため、差分レビューでは気づけない。

そこで「何が無いか」を出力ではなく構造から見るテストを足した。

| | 対象 | 落ちる条件 |
|---|---|---|
| ブロック網羅 | 自前のブロック定義49個 | 一度も astGenerator を通らないブロックがある |
| 文脈網羅 | AST の入れ子 | 同じノード型がある文脈でだけ痩せている |

#### 見つかったもの

**自前ブロック49個のうち22個が、レイヤーBで一度も生成されていなかった。**
形容詞ブロック5種・動詞カテゴリ4種・TimeChip 3種・疑問ブロック3種などが
まるごと astGenerator を通っていない状態だった。22件すべてケースを追加した。

これを通したところ、**英語が `location` の項に前置詞を付けない**のが判明した。

```
I live a park.     ← "I live in a park."
I reside a park.   ← "I reside in a park."
I stay a park.     ← "I stay at a park."
```

日本語も location を一律「で」にしており、住む・居住する・滞在するは
「に」が正しい（「公園で住む」→「公園に住む」）。`put` / `place` / `hang` の
`goal` も同じく前置詞が落ちる。**未修正**（KNOWN BUG として固定）。

レイヤーAの文脈の穴7件は動作自体は正しかったので、ケースだけ追加した。

#### 構成

レイヤーBのケース表を `astGenerator.cases.ts` に切り出した。テスト本体と
網羅チェックの両方から読むため（レイヤーAの `cases.ts` と同じ形になった）。

文脈網羅は0件を目指すものではない。残り23件はほとんどが限定詞の値の
組み合わせで、潰しても得るものが少ない割に表が膨らむ。**増やさないこと**が目的。
