...
- [ ] 英語が location / goal の項に前置詞を付けない（`I live a park.`）
      - `live in` / `reside in` / `stay at` は動詞ごとに決まるので語彙データが要る
      - `put` / `place` / `hang` の `goal` は前置詞を利用者が選ぶ（in / on / under）ため、
        裸の NP スロットではなく前置詞句スロットにすべきか要検討
      - 日本語も location を一律「で」にしている（住む・滞在するは「に」）
