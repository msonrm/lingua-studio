...
- [x] **等位接続の統一**: VP 等位接続を n項ツリーにし、カンマ・correlative を構造から決定 — 2026-07-27 完了
  - `CoordinatedVerbPhraseNode` を導入（NP の `CoordinatedNounPhraseNode` と対称）
  - 左入れ子 `or(and(A,B),C)` と右入れ子 `and(A,or(B,C))` を区別できるようになった
  - `NOT` のオペランド構築も二項演算子と統一
...
- [x] モダリティの迂言形式が単純相にしか対応しておらず英語が壊れる — 2026-07-27 修正
- [x] `AdjectivePhraseNode.degree` を英語レンダラーが無視する — 2026-07-27 修正
      （3レンダラーすべてが扱えるようになった。ブロック追加はまだ）
- [x] 副詞ラッパーのラベル行スキップが到達不能な防御コード — 2026-07-27 削除
- [x] 日本語は動詞否定とモダリティ否定を表層で区別しない
      → 日本語の性質上そうなるのが自然と判断し、仕様として受け入れる
- [x] 可算名詞の単数が限定詞なしで出力される（`I am biggest man.`）— 2026-07-27 修正
      最上級の DET 自動補正（`a` → `the`）と、限定詞欠損の `___` 表示
- [x] 日本語で VP 等位接続の2つ目以降の項が消える — 2026-07-27 修正
      先頭の動詞句からしか項を取っていなかった
- [x] 英語で同一動詞の VP 等位接続が不自然（`I have a pen and have an apple.`）
      — 2026-07-27 修正。同一動詞のときは主語を繰り返す

## 実験の結果を受けて

- [x] LinguaScript の実用性検証 — 2026-07-27。**実用性は示せなかった**
      （[docs/EXPERIMENT-RESULT.md](./docs/EXPERIMENT-RESULT.md)）
- [ ] 方向性の再検討。実験は「曖昧さを AI に伝える」路線を否定したが、
      「曖昧さを人間に見せる」路線（元々の出自）は否定していない
      - Phase 0 の衝突一覧は LLM 抜きで作れ、そのまま教材の素材になる
      - 日本語のほうが曖昧という結果は、日本語側の可視化に使えるかもしれない

## 文法機能

- [x] 比較級・最上級（級のみ）— 2026-07-27
- [ ] 比較対象（than 句）— `post:than('that_one)` として後置修飾で表す予定
- [x] 英語が location / goal の項に前置詞を付けない（`I live a park.`）— 2026-07-27
      前置詞動詞（`belong to`）は1語として扱い、AST には出さない。
      利用者が選ぶもの（`live in`）は欠けを `___` で示すところまで
- [ ] `choice` の前置詞を実際に選べるようにする
      - いまは `I live ___ a park.` と欠けを示すだけで、満たす手段が無い
      - 案: 動詞ブロックの項スロットに前置詞のドロップダウンを付け、
        `FilledArgumentSlot.preposition` として AST に載せる
      - 前置詞は意味を担うので LinguaScript には出す（`fixed` とは逆）
- [ ] 限定詞欠損を Grammar Console の警告としても出す
      現状は英語出力に `___` が出るだけで、なぜ非文なのかの説明が無い
- [ ] 願望（〜たい）・証拠性（〜そうだ）— 実装するかは判断次第
- [ ] 日本語の丁寧形（Register）・敬語 — 優先度低

## 辞書構造の見直し

- [x] 第1段階: `languages/` への移設と言語パックの契約定義 — 2026-07-27
- [x] 第2段階: 保存形式の刷新（正式版 1）と移行、日本語への配線 — 2026-07-27
- [x] 第3段階: `DictionaryPanel` の入力欄を言語パックから動的生成 — 2026-07-27
- [x] 第5段階: 形容詞・副詞をユーザー辞書から使えるように — 2026-07-27
- [ ] 第4段階: 言語追加時の欠落可視化（`unverified` を UI に出す）と `derive`
