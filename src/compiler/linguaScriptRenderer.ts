...
function renderVerbPhraseToScript(vp: VerbPhraseNode): string {
  ...
  for (const pp of vp.prepositionalPhrases) {
    const objScript = renderNounPhraseToScript(pp.object);
...
function renderFillerToScript(filler: NounPhraseNode | AdjectivePhraseNode): string {
...
function renderNounPhraseToScript(np: NounPhraseNode): string {
  ...
  // 前置詞句修飾
  if (np.prepModifier) {
    const objScript = renderNounPhraseToScript(np.prepModifier.object);
